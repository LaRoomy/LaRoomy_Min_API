        ...
        if (propertyIndex < propertyCount)
        {
            ...
            case PTYPE_UNLOCK_CONTROL:
                if (length >= 12)
                {
                    ...
                    {
                        uint8_t result = executionStringToUnlockControlStruct(data, unlockControlState);
                        if (rmtCallback != NULL)
                        {
                            if (result == UNLOCKCTRL_LOCK_SUCCESS || result == UNLOCKCTRL_PIN_CHANGE_SUCCESS || result == UNLOCKCTRL_UNLOCK_SUCCESS)
                            {
                                // report state change
                            ...
                            else
                            {
                                ...
                                responseData.a = result;
                                rmtCallback(propertyID, REVT_UNLOCK_CONTROL_INVALID_OPERATION);
                            }
                        }

                        // generate response (state update)
                        if (result == UNLOCKCTRL_LOCK_SUCCESS || result == UNLOCKCTRL_PIN_CHANGE_SUCCESS || result == UNLOCKCTRL_UNLOCK_SUCCESS)
                        {
                            unlockControlState->flags = UCFLAG_SUCCESS;
                        }
                        else if (result == UNLOCKCTRL_LOCK_ERROR || result == UNLOCKCTRL_UNLOCK_REJECTED_NO_ACCESS)
                        {
                            unlockControlState->flags = UCFLAG_FAIL_UNEXPECTED;
                        }
                        else if (result == UNLOCKCTRL_UNLOCK_REJECTED_WRONG_PIN)
                        {
                            unlockControlState->flags = UCFLAG_UNLOCK_FAILED_PIN_REJECTED;
                        }
                        else if (result == UNLOCKCTRL_PIN_CHANGE_FAIL_WRONG_PIN)
                        {
                            unlockControlState->flags = UCFLAG_PIN_CHANGE_FAILED_OLD_KEY_INVALID;
                        }
                        else if (result == UNLOCKCTRL_PIN_CHANGE_FAIL_NO_ACCESS)
                        {
                            unlockControlState->flags = UCFLAG_PIN_CHANGE_FAILED_NO_ACCESS;
                        }
                        else
                        {
                            unlockControlState->flags = UCFLAG_FAIL_UNEXPECTED;
                        ...
                        char stateString[15] = {0};
                        unlockControlStateToStateResponseString(unlockControlState, stateString);
                        sendData(stateString);
                    }
                }
                break;
